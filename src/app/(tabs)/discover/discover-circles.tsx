...
interface RallyData {
  ...
  requestPending?: boolean; // Track if user has a pending request
}
...
export function DiscoverRallies() {
    ...
    try {
      ...
      if (response.ok) {
        if (result.status === "joined") {
          ...
          toast.success(result.message || "Joined circle!");
          setData((prev) =>
            prev
              ? {
                  ...prev,
          ...
          setData((prev) =>
            prev
              ? {
                  ...prev,
                  circles: prev.circles.map((c) =>
                    c.id === circleId ? { ...c, requestPending: true } : c
                  ),
                  trending: prev.trending.map((c) =>
                    c.id === circleId ? { ...c, requestPending: true } : c
                  ),
  ...
  const getJoinButtonConfig = (circle: RallyData) => {
    ...
    if (circle.requestPending) {
      return { text: "Pending", disabled: true, variant: "outline" as const, icon: Clock };
    }
    if (circle.joinType === "invite_only") {
      return { text: "Invite Only", disabled: true, variant: "outline" as const, icon: Lock };
    }
    if (circle.joinType === "request") {
      return { text: "Request", disabled: false, variant: "default" as const };
    }
    // Default: open
    return { text: "Join", disabled: false, variant: "default" as const };
  };
                        ...
                        >
                          <Card
                            className={cn(
                              ...
                              circle.isMember && "ring-2 ring-brand/30"
                            )}
                            onClick={() => setSelectedRallyId(circle.id)}
                          >
                            <CardContent className="p-4 text-center">
                              <div className="relative mx-auto w-fit mb-2">
                                <Avatar className="h-14 w-14">
                                  <AvatarImage src={circle.imageUrl || undefined} />
                              ...
                              {!circle.isMember && (() => {
                                const btnConfig = getJoinButtonConfig(circle);
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="font-medium text-muted-foreground">
                {searchQuery ? "No circles found" : "No circles available"}
              </p>
                ...
                return (
                  <Card
                    ...
                    className={cn(
                      ...
                      circle.isMember && "border-brand/30"
                    )}
                    onClick={() => setSelectedRallyId(circle.id)}
                  >
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1.5">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {circle.memberCount.toLocaleString()} member
                          ...
                          return (
                            ...
                            >
                              {joiningRally === circle.id ? (
                                ...
                                <>
                                  {BtnIcon ? (
                                    <BtnIcon className="h-4 w-4 mr-1" />
                                  ) : btnConfig.text === "Join" ? (
                                    <Plus className="h-4 w-4 mr-1" />
                                  ) : null}
                                  {btnConfig.text}
